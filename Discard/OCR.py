...
# #
# # img_ori = cv2.imread(img_path)#cv2.imread('LP_OCR/1.jpg')
...
#             return None
#         elif event.src_path.endswith((".jpg", ".png")):
...
#         new_folder_path = os.path.join('LP_OCR', current_date)
#         os.makedirs(new_folder_path, exist_ok=True)
#         new_image_name = os.path.join(new_folder_path,strftime("%d일-%H시%M분%S초") + chars + '.png')
#         cv2.imwrite(new_image_name, img_out)
...
def process_image(img_path):
    ...
    os.remove(img_path)  # delete the old image
    current_date = datetime.datetime.now().strftime('%Y_%m_%d')
    new_folder_path = os.path.join('home/dev/Capstone/LP_OCR', current_date)
    os.makedirs(new_folder_path, exist_ok=True)
    new_image_name = os.path.join(new_folder_path, strftime("%Y년%m월%d일-%H시%M분%S초") + chars + '.png')
    cv2.imwrite(new_image_name, img_out)
    print(f"Image saved as {new_image_name}")
